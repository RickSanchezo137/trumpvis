...
class TrumpvisApplicationTests {
    ...
    @Test
    void contextLoads(){
        System.out.println(iTweetsService.help_vis_relation("2019-10", "2019-11"));
    }

    @Test
    void saveWordCloud2Redis(){
        String time = "2009-05";
        Date end = StringDateTransferUtils.str2date_YM("2020-01", 1);
        Date temp;
        while(StringDateTransferUtils.str2date_YM(time, 0).compareTo(end) != 0){
            for(int i = 0; (temp = StringDateTransferUtils.addMonthFromStr(time, i)).compareTo(end) != 1; i++){
                iTweetsService.help_vis_wordcloud(time, StringDateTransferUtils.date2str_YM(temp));
